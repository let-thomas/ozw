...
https://raw.githubusercontent.com/NextDom/plugin-ozw672/master/core/class/ozw672.class.php
 *
...
class ozw672 extends eqLogic
{
            ...
            if (isset($obj['MenuItems'])) {
                foreach ($obj['MenuItems'] as $item) {
                    ...
                    if (!is_object(self::byLogicalId($item['Id'], 'ozw672'))) {
                        ...
                        $eqLogic = (new ozw672())
                                ->setLogicalId($item['Id'])
                                ->setName($item['Text']['Long'])
                                ->setEqType_name('ozw672')
                                ->setConfiguration('type', 'appareil')
                                ->setConfiguration('address', substr($item['Text']['Long'], 0, strpos($item['Text']['Long'], " ")))
    ...
    public function postInsert()
    {
        if ($this->getIsEnable()) {
            ...
            if (!is_object($cmd)) {
                $cmd = (new ozw672Cmd())
                        ->setName('Dernier refresh')
                        ->setEqLogic_id($this->getId())
                        ->setLogicalId('updatetime')
                        ->setUnite('')
                        ->setType('info')
                        ->setSubType('string')
                        ->setIsHistorized(0)
